...
tk.Label(root, text="Jogo da Velha", font=('Ariel', 25)).pack()
status_label = tk.Label(root, text="Vez do: X", font=('Ariel', 15), bg='gray', fg='snow')
...
def play_again(): 
    ...
    for point in XO_points: 
        ...
        point.reset()
    status_label.configure(text="Vez do: X")
    play_again_button.pack_forget()


play_again_button = tk.Button(root, text='Jogar Novamente', font=('Ariel', 15),
command=play_again)
    ...
    def set(self):
        ...
        if not self.value:
            ...
            if current_chr == "X":
                ...
                current_chr = "O"
                status_label.configure(text="Vez do: O")
            elif current_chr == "O":
...
def check_win():
    for possibility in winning_possibilities:
        if possibility.check('X'):
            status_label.configure(text="X Venceu!")
            disable_game()
            return
        elif possibility.check('O'):
            status_label.configure(text="O Venceu!")
            disable_game()
            return
    if len(X_points) + len(O_points) == 9:
        status_label.configure(text="Deu velha!")
        disable_game()
